...
export const signUp = actionGeneric({
  ...
  handler: async (ctx, { email, password }): Promise<string> => {
    ...
    const existing = await ctx.runQuery((anyApi as any).users.getByEmail, { email: e });
    if (existing !== null) {
      throw new Error("An account with that email already exists.");
    }
    const id = await ctx.runMutation((anyApi as any).users.create, {
      email: e,
      passwordHash: await hashPassword(password),
    });
    return signJwt(id as string, { claims: { email: e } });
  },
});
...
export const signIn = actionGeneric({
  ...
  handler: async (ctx, { email, password }): Promise<string> => {
    const e = normalizeEmail(email);
    const user = await ctx.runQuery((anyApi as any).users.getByEmail, { email: e });
    if (
      !user ||
      !user.passwordHash ||
...
export const signInAnonymous = actionGeneric({
  ...
  handler: async (ctx): Promise<string> => {
    const id = await ctx.runMutation((anyApi as any).users.create, {});
    return signJwt(id as string, { claims: { anonymous: true } });
  },
});
